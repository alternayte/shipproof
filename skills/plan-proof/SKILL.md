---
name: plan-proof
description: Map each ShipProof requirement to a runnable proof in verification.json. Use when a ShipProof requirement needs a proof, when filling in verification.json, when `shipproof status` reports that a requirement needs a proof, or when deciding whether a requirement needs a human proof.
---
...
Proofs live in `.shipproof/changes/<change-id>/verification.json`. Open it and
give every requirement at least one proof.
...
```json
{
  ...
  "requirements": [
    {
      "id": "SP-1-R1",
      "statement": "Reject a request over the configured limit",
      "proof": [
        {
          "type": "command",
          "target": "./internal/gateway",
          "command": "go test -run TestRejectsOverTheLimit ./internal/gateway/"
        }
      ]
    },
    {
      "id": "SP-1-R2",
      "statement": "The rejection message reads well to a customer",
      "proof": [
        {
          "type": "human",
          "target": "the rejection message",
          "human": true,
          "rationale": "No command can judge whether a sentence reads well.",
          "accepted_by": "",
          "accepted_at": ""
        }
      ]
...
`target` names what the proof judges. `command` runs it. A requirement may hold
more than one proof, and all of them must pass.

A proof is one form or the other. A `command` proof carries no `human: true`,
and a human proof carries no command. A human proof needs a `rationale` that
says why no command can judge it, so a reader can challenge the choice.

Leave `accepted_by` and `accepted_at` empty. A person fills them when they
accept the human proof, and ShipProof then grades it `stated`.

Ask ShipProof what is still missing:

```
shipproof status <change-id>
```

It names every requirement with no proof, and every proof whose program is not
installed.
