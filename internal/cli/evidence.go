...
func runEvidence(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "usage: shipproof evidence pack <change-id> [--base <rev>] [--head <rev>]")
		return 2
	}
	...
	case "review":
		return runEvidenceReview(stderr)
	default:
...
func runEvidenceReview(stderr io.Writer) int {
	fmt.Fprintln(stderr, "shipproof evidence review is gone. ShipProof needs no GitHub credential.")
	return 2
}
