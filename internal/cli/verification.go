...
// checkPlan validates the verification plan of one change and ties it to the
// requirement set. `prove` runs it before the gate, so a plan that cannot
// judge a requirement never hides behind a passing gate.
func checkPlan(root, changeID string, stdout, stderr io.Writer) int {
	path := verification.Path(root, changeID)
	...
	if requirements.Exists(root, plan.ChangeID) {
